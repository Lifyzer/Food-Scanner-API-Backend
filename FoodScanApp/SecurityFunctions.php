...
class SecurityFunctions
{
    ...
    public function expiredAllTokenofUser($userData)
    {
        ...
        if ($user_id != '') {
            ...
            $updateQuery = "update " . TABLE_APP_TOKENS . " set  modified_date = '" . $modifiedDate . "' where userid = '" . $user_id . "' ";
        ...
        if ($user_id != '') {
            ...
            if ($stmt_get_config = $connection->prepare($query)) {
                ...
                if ($stmt_get_config->num_rows > 0) {
                    $val = fetch_assoc_stmt($stmt_get_config);
                    $expiryDuration = $val[0]['config_value'];
                    $currentdate = date("dmyHis", time() + $expiryDuration);
                    $insertUpdateQuery = "INSERT INTO " . TABLE_APP_TOKENS . " (userid,token,expiry) VALUES(?,?,?)
            ON DUPLICATE KEY UPDATE token = ? , expiry = ?, created_date = ?";
                    if ($insert_update_query_stmt = $connection->prepare($insertUpdateQuery)) {
                        $insert_update_query_stmt->bind_param('isssss', $user_id, $generateToken, $currentdate, $generateToken, $currentdate, $modifiedDate);
                        if ($insert_update_query_stmt->execute()) {
                            $insert_update_query_stmt->close();
                            ...
                            if ($stmt_get_global_pwd = $connection->prepare($query_global_pwd)) {
                                ...
                                if ($stmt_get_global_pwd->num_rows > 0) {
                                    ...
                                    while ($val = fetch_assoc_all_values($stmt_get_global_pwd)) {
                                        ...
                                        $hitesh = 'hitesh';
                                        $data['hitesh'] = $security->encrypt($hitesh, $masterKey);
                                    }
                                }
                            }
                        ...
                        // echo "prepare app_token failed".$insert_update_query_stmt->error;
                    }
                } else {
...
//                                                            echo $tempToken . " == " . $secretvalue . " == ".$masterKey;
//
...
//                    echo "\ndecrpt => ".$decrypted_access_key;
                    ...
                    $queryToCheckAccessKeyExist = "SELECT * FROM " . TBL_USERS . " WHERE guid ='" . $decrypted_access_key . "' AND is_delete='" . DELETE_STATUS::NOT_DELETE . "'";
                    if ($stmt_check_access_key = $connection->prepare($queryToCheckAccessKeyExist)) {
                        ...
                        if ($stmt_check_access_key->num_rows > 0) {
                            ...
                            while ($user_value = fetch_assoc_all_values($stmt_check_access_key)) {
                                ...
                                $queryToCheckRecordExist = "SELECT * FROM " . TABLE_APP_TOKENS . " WHERE userid =" . $user_value['id'] . " AND is_delete='" . DELETE_STATUS::NOT_DELETE . "'";
