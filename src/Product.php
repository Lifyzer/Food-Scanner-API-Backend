...
class Product
{
    ...
    public function addReview($userData)
    {
        ...
        editData($connection, "updateReview", TABLE_REVIEW, ['is_delete' => DELETE_STATUS::IS_DELETE], ['user_id' => $user_id,'product_id' => $product_id,is_test =>$is_testdata,is_delete => IS_DELETE], "");
     ...
     public function removeProductFromHistory($userData)
    {
        ...
        $edit_history_response = editData($connection, "removeProductFromHistory", TABLE_HISTORY, ['is_delete' => $is_delete], ['id' => $history_id , 'is_test' =>$is_testdata], "");
        if ($edit_history_response[STATUS_KEY] === SUCCESS) {
            $objHistory = getSingleTableData($connection, TABLE_HISTORY, '', '*', '', ['id' => $history_id,'is_test' =>$is_testdata,'is_delete' => $is_delete]);
            if (!empty($objHistory)) {
    ...
    public function getAllUserFavourite($userData)
    {
        ...
        $select_user_favourite_stmt = getMultipleTableData($connection, "", $select_user_favourite_query, "", "", []);
